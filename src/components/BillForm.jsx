...
export default function BillForm({
  ...
  const renderFixedCard = (itemKey, title, tooltipTitle) => (
    <div className="bill-input-card">
      <div className="input-group">
        <label className="input-label">
          ...
          <button 
            type="button" 
            className={`btn-toggle-config-inline ${splits[itemKey]?.length !== 2 ? 'customized' : ''}`}
            onClick={() => toggleExpand(itemKey)}
            title={`Atur pembagian ${tooltipTitle}`}
          >
            ⚙️ {splits[itemKey]?.length || 0} Orang
          </button>
        </label>
          ...
          {isEditingFixed ? (
            <>
              ...
              <input
                type="number"
                value={bills[itemKey] || ''}
                onChange={(e) => handleBillChange(itemKey, e.target.value)}
                placeholder="0"
              />
            </>
          ) : (
            <div className="fixed-value-display">
              {formatIDR(bills[itemKey])}
            </div>
          )}
        </div>
      </div>
      
      {expandedSection === itemKey && (
        <div className="split-config-drawer animate-fade-in">
          <span className="drawer-title">{title} dibagi ke:</span>
          <div className="selector-grid">
            {members.map(m => (
              <label key={m.id} className={`selector-chip ${splits[itemKey]?.includes(m.id) ? 'active' : ''}`}>
                <input
                  type="checkbox"
                  checked={splits[itemKey]?.includes(m.id)}
                  onChange={() => handleToggleMember(itemKey, m.id)}
                />
                <span>{m.name}</span>
              </label>
            ))}
  ...
  return (
    <div className="bill-form-panel">
      ...
      {/* --- VARIABEL BILLS --- */}
      <div className="bill-group-section">
        <h4 className="bill-group-title">⚡ Biaya Variabel (Berubah per Bulan)</h4>
      ...
      {/* --- FIXED BILLS --- */}
      <div className="bill-group-section" style={{ marginTop: '1.5rem' }}>
        <div className="bill-group-header">
          <h4 className="bill-group-title">📌 Biaya Tetap (Fixed)</h4>
          <button 
            type="button" 
            className="btn-edit-fixed"
            onClick={() => setIsEditingFixed(!isEditingFixed)}
          >
            {isEditingFixed ? 'Selesai Edit' : 'Edit Biaya'}
          </button>
        </div>

        {renderFixedCard('room', 'Sewa Kamar (Rent)', 'kamar')}
        {renderFixedCard('ac', 'Sewa AC (Air Conditioner)', 'AC')}
        {renderFixedCard('internet', 'Internet / Wifi', 'internet')}
        {renderFixedCard('tax', 'Pajak (Tax)', 'pajak')}
      </div>
    </div>
  );
}
